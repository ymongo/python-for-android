...
class Python2LegacyRecipe(TargetPythonRecipe):
    ...
    patches = ['patches/Python-{version}-xcompile.patch',
               ...
               ('patches/disable-openpty.patch',
                check_all(is_api_lt(21), is_ndk('crystax')))]
    ...
    def build_arch(self, arch):
        ...
        if not exists(self.ctx.get_python_install_dir()):
            shprint(sh.cp, '-a',
                    join(self.get_build_dir(arch.arch), 'python-install'),
        ...
        if not exists(join(self.ctx.get_libs_dir(arch.arch),
                           'libpython2.7.so')):
            shprint(
                sh.cp, join(self.get_build_dir(arch.arch), 'libpython2.7.so'),
        ...
        #     if not exists(join(self.ctx.get_python_install_dir(),
        #                        'libpython2.7.so')):
        #         info('Copying python-install to dist-dependent location')
        #         shprint(sh.cp, '-a', 'python-install',
        #                 self.ctx.get_python_install_dir())
        #     self.ctx.hostpython = \
        #         join(self.ctx.get_python_install_dir(), 'bin', 'python.host')
        #     return
    ...
    def do_python_build(self, arch):
        ...
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            try:
                ...
                print('First python2legacy make failed.'
                      ' This is expected, trying again.')
            ...
            # shprint(sh.cp, self.ctx.hostpython,
            #         join(self.ctx.get_python_install_dir(),
            #              'bin', 'python.host'))
            # self.ctx.hostpython = \
            #     join(self.ctx.get_python_install_dir(), 'bin', 'python.host')
